...
デプロイ後は実際に画面が開くことを確かめる（https://frestyle.jp）。

**ただし現在このデプロイは通らない。** `cd-frontend.yml` は認可の設定（`VITE_OIDC_AUTHORIZE_URI` / `VITE_OIDC_CLIENT_ID`）が空なら Build を止める。本番の認証発行者がまだ無く、Secrets が空のため。ここを素通しすると、**押しても何も起きないログインボタンを本番に出す**ことになるので、止まるのが正しい。あわせて `api.frestyle.jp` は DNS ごと消えており（バックエンド撤去のため）、API を呼ぶ画面はいずれにせよ動かない。

出せるようにするには、先に本番の発行者を立てて Secrets を入れる。**この検査を外して出すという解き方はしない。**

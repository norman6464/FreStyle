...
### 5.0 デプロイは常に両方（片方だけ変えたときも）

**バックエンドとフロントエンドのどちらか一方しか変更していなくても、デプロイは必ず両方走らせる。**
...
- 変更が無い側も走らせてよい（同じ内容がもう一度出るだけ）
- **どちらかを出せない事情があるなら、出せる側も止める。** 片方だけ進めない
- 出せない事情（インフラ側の対応待ちなど）があるときは、**何がブロックしているかをチケットに書き、解消後に両方を出す**
- デプロイ後は両方の生存を確かめる。バックエンドは `GET /api/v2/health`、フロントエンドは実際の画面が開くこと

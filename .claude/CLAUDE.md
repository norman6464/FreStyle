...
> **アーキテクチャ仕様の一次情報は [`docs/ARCHITECTURE.md`](../docs/ARCHITECTURE.md)**。このファイルは必ず整合を保つこと。
...
- handler を追加 / 変更する PR では**必ず `make openapi`** を走らせ `docs/` の差分も commit する
...
- 理由（実際の事故）: PR 4 本同時 open で `docs/swagger.*` 等の生成ファイルが互いにコンフリクトし管理コストが跳ね上がった
...
- **取り組んだもの・実装内容・手順は必ず `docs/` に書く。ドキュメント更新が無い PR はマージ禁止**（軽微なタイポ修正など明らかに不要な場合を除く）
- 新機能・画面 → 使い方 / 設計判断を docs に。新しい usecase / repository / handler / Component → `docs/ARCHITECTURE.md` を更新。失敗からの知見 → トラブルシューティング系 docs に追記。CI/CD・運用フロー → `.github/workflows/README.md` 等を更新
- docs は**実装と同じ PR 内**で書く（後日 docs PR は不可。Documentation-First は歓迎）
- 「なぜやったか」「どう動かすか」「何が変だったか」を残し、半年後に同じ作業を再現できる状態を保つ
...
- **構成**: `courses/NN-{course-slug}/` に `course.yaml`（title / description / category / sort_order / is_published。**コース ID は含まれない**）+ `001-{slug}.md` 連番の教材。教材は `# タイトル` から始める（先頭 h1 が DB の title になる）
...
- **落とし穴 1（全削除）**: `{course}__course.sql` は course を UPSERT 後にその course の `teaching_materials` を **DELETE** する冪等設計。1 章だけ追加する場合も**そのコースの SQL 全件**を再 apply しないと過去章が消える。適用後は本番 UI で章数を確認する
- **落とし穴 2（黙ってスキップ）**: `seed-courses.py` の `SORT_TO_ID`（sort_order → 本番コース ID の手書き対応表）に無い sort_order は、**エラーにならず** `skip (unknown sort_order N)` の警告付きでスキップされ SQL が生成されないまま正常終了する（終了コード 0）。新規コース追加時は `SORT_TO_ID` への追記を忘れず、生成された SQL のファイル数を確認する
- **最新のコース一覧・id 対応は教材リポの `courses/` と `SORT_TO_ID` を一次情報として参照する**（このファイルに一覧を複製しない — 陳腐化して誤情報の元になるため）
...
9. 層設計の詳細は [`docs/ARCHITECTURE.md`](../docs/ARCHITECTURE.md) を一次情報として参照

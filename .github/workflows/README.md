...
| クロスリポ | **`IAC_REPO_TOKEN`** | `cd-backend.yml` が IaC リポ（非公開）（プライベート）から CFn テンプレートを取得するための PAT |

### `IAC_REPO_TOKEN` のセットアップ

`cd-backend.yml` は ECS スタックを CloudFormation でデプロイするため、IaC リポ（非公開） リポジトリから CFn テンプレートを clone します。プライベートリポジトリなので、Fine-grained Personal Access Token (PAT) が必要です。
...
3. Repository access: `Only select repositories` → IaC リポ（非公開）

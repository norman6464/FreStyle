...
- **usecase（単体）**: repository / infra を `testify/mock` で差し替え、ビジネスロジックを隔離検証。
- **handler（結合）**: `httptest` + `gin.New()` でルータ〜ステータス〜JSON を通しで検証。
- **middleware（単体）**: JWT 認証・CurrentUser 注入・レートリミット等の横断処理。
- **infra（単体 / 境界）**: Cognito / JWKS / oEmbed / SES などの外部 I/O を境界で fake / httptest 化。
- **repository（結合）**: `adapter/persistence` 層を **本物の PostgreSQL** に対して検証する。`//go:build integration` で隔離し、`make test-integration`（compose 起動 → 実行 → teardown）で回す。通常の `go test ./...` からは独立で、実 SQL の WHERE / ORDER / 制約を検証する。
- **自作 linter（単体 + 結合）**: `archlint` / `apispec-lint` / `naminglint` を、純粋関数（単体）と一時ツリー走査（結合）で検証。
...
> **カバレッジ閾値ゲート**: frontend は `vitest.config.js` の `coverage.thresholds`、backend は総計 floor（`COVERAGE_MIN`）を下回ると CI を fail させる。backend は domain / 生成 docs など 0% 群を含む総計のため低めの floor から始め、テスト追加に合わせてラチェット的に引き上げる。
...
- **SAST / 依存 / 脆弱性**: CodeQL（Go / TypeScript）、govulncheck、**Trivy**（依存 CVE / Dockerfile 誤設定）、**Dependabot**（依存・Actions の自動更新）。

詳細・セットアップ手順は [CONTRIBUTING.md](./CONTRIBUTING.md) §6〜§8 を参照。

## ステージング / リリースフロー

本番反映の前に、**ステージング（Lightsail 想定）でリリース前確認**を挟む。`Makefile`（リポジトリ直下）+ `scripts/` で完結する。

```bash
# 1) ステージングへデプロイ（SSH → git pull → build → 再起動）
make staging-deploy STAGING_HOST=ec2-user@<lightsail-ip> STAGING_SSH_KEY=~/.ssh/staging.pem BRANCH=main

# 2) リリース前確認（死活 / 認証境界のスモーク。NG なら非ゼロ終了）
make staging-verify STAGING_URL=https://<staging-url>

# 1)+2) 一括（検証 NG で停止）
make staging-release STAGING_HOST=... STAGING_URL=...

# 3) 検証 OK なら本番反映を起動（GitHub の production Environment 承認ゲートで停止 → 承認で反映）
make prod-deploy-backend     # CD - Backend Deploy to ECS
make prod-deploy-frontend    # CD - Frontend Deploy to S3 + CloudFront
```

- `scripts/staging-deploy.sh` … SSH 先で `git pull` → `docker-compose.staging.yml` で再ビルド/再起動（Lightsail の実構成に合わせて編集）。
- `scripts/staging-verify.sh` … `/`・`/api/v2/health`・認証必須 401 を検査。
- **二段ゲート**: ステージング検証で通った後でも、本番反映は `production` Environment の**人間の承認待ち**で停止する。

> Lightsail は導入検討中のため、スクリプトは host を env で指定する**雛形**。プロビジョン後に SSH / 再起動部分を実構成へ合わせる。

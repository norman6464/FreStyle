...
> **カバレッジ閾値ゲート**: frontend は `vitest.config.js` の `coverage.thresholds`（lines 85 / statements 85 / functions 80 / branches 78）で、現状（lines 88.6% 等）を下回ると CI を fail させる。backend は総計 floor（`COVERAGE_MIN`、現状 45.2% に対し 44%）。backend は domain / 生成 docs / `cmd/server` / 薄い infra など 0% 群を含む総計のため低めの floor で、テスト追加に合わせてラチェット的に引き上げる（handler テスト追加で 39.2%→45.2%、floor 38→44）。

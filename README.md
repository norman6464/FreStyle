...
| ワークフロー | 実行内容 |
|---|---|
| `ci-backend-go.yml` | `go vet` / 自作 linter（`archlint`・`apispec-lint`・`naminglint`）/ `go test -race`（単体・カバレッジ計測）/ `go build`、および **本物の PostgreSQL に対する結合テスト**（`go test -tags=integration`）|
...
| `codeql.yml` / `trivy.yml` / `secret-scan.yml` | SAST（Go / TypeScript）/ 依存 CVE・Dockerfile 誤設定 / gitleaks シークレットスキャン |

> **カバレッジ閾値ゲート**: frontend は `vitest.config.js` の `coverage.thresholds` を下回ると CI を fail させる。backend は `go test -race -covermode=atomic` のカバレッジプロファイルで計測し、新規追加コードは 80% 以上を目標にする。

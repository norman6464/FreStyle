...
- **本番デプロイの承認ゲート**: マージ即本番反映ではない。**backend は CodePipeline + CodeDeploy（ECS Blue/Green）** で、green を **test listener で検証してから手動でトラフィック移行**（Canary + 5xx 自動ロールバック・ダウンタイムゼロ）。**frontend は `production` Environment**（required reviewers = `@norman6464`）の承認待ちで停止。手順詳細は IaC リポの `docs/30`。
- **シークレット漏洩の多層防御**:

...
AI チャットの SSE ストリーム(Bedrock) / 演習(サンドボックス実行) / SES 連携。CSRF 対策。

AI チャットは **セッション(メタデータ=RDS) + メッセージ読取(DynamoDB) + 添付(S3 presigned)** を
移植した。新規メッセージを書く SSE ストリーム(Bedrock Converse)は別 PR。セッション / メッセージは
取得・更新・削除で所有者検証を行い、他人のものは 403（Go 版に無かった IDOR 対策を追加）。メッセージは
DynamoDB(`frestyle.dynamo.ai-chat-table` = `DYNAMODB_AI_CHAT_TABLE`、PK=sessionId / SK=messageId)を
Query して作成順で返す。table 未設定時は空リストを返す stub にフォールバック。添付は画像のみ許可・
5MB 上限を service で検証し、未対応 MIME は 415 / サイズ超過は 413。キーは `ai-chat/{userId}/{uuid}.{ext}`。

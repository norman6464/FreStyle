...
| `POST /api/v2/code/execute` | コード実行サンドボックス（Java / stdout・stderr・exitCode） | 必須 |
...
**コード実行サンドボックス** (`POST /api/v2/code/execute`): ユーザーの Java コードを子プロセスで実行し
...
対応言語は現状 java のみ（php/go は将来 `ProcessCodeExecutor` に追加）。

...
AI チャットのメッセージ(DynamoDB) / SSE ストリーム(Bedrock) / 添付(S3) / 演習(サンドボックス実行) /
SES 連携。CSRF 対策。

AI チャットは **セッション(メタデータ=RDS) + 添付(S3 presigned)** を移植した。メッセージ本体
(DynamoDB)と SSE ストリーム(Bedrock)は別 PR。セッションは取得 / 更新 / 削除で所有者検証を行い、
他人のものは 403（Go 版に無かった IDOR 対策を追加）。添付は画像のみ許可・5MB 上限を service で
検証し、未対応 MIME は 415 / サイズ超過は 413。キーは `ai-chat/{userId}/{uuid}.{ext}`。

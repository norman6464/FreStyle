...
| `GET /api/v2/exercises` | master 演習一覧（公開のみ / `?language=` で絞り / current user の status + 集計 stats 付き） | 必須 |
| `GET /api/v2/exercises/{slug}` | 演習詳細（本体 + 入出力例） | 必須 |
| `POST /api/v2/exercises/{slug}/submit` | 提出をサーバ側で採点（execute=実行して stdout 比較 / qa=提出文字列比較）し履歴保存 | 必須 |
| `GET /api/v2/exercises/{slug}/submissions` | 自分の提出履歴（新しい順） | 必須 |
| `GET /api/v2/company/settings` | 自社設定（`aiChatEnabledForTrainees`）取得 | company_admin / super_admin |
| `PUT /api/v2/company/settings` | 自社の trainee への AI 有効化を更新 | company_admin / super_admin |
...
**会社設定 / trainee の AI 有効化**: `companies.ai_chat_enabled_for_trainees`（V13、既定 TRUE）を会社単位で持ち、
company_admin / super_admin が `GET/PUT /api/v2/company/settings` で切り替える。`AiChatAccessPolicy` +
`AiChatAccessInterceptor`（`/api/v2/ai-chat/**`）で、無効会社の **trainee は AI チャット API を 403** で遮断
（UI 非表示だけに頼らずサーバ側で認可）。管理者は設定に関わらず利用可。`GET /api/v2/auth/me` に
`aiChatEnabledForTrainees` を含め、frontend はサイドバーの「AI」表示判定に使う。設計は Design Doc #1846。

**演習（master_exercises）**: 一覧 / 詳細 / 提出採点 / 提出履歴を移植した。採点ロジックは
**サーバ側に一本化**（`ExerciseGrading` + `SubmitExerciseUseCase`）。従来 frontend にも同等の
`normalizeOutput` があったが、採点の正本はテスト可能な backend に置く。`mode` で分岐し、
`execute` は `master_exercise_examples` を全件 `CodeExecutor` で実行し各 `inputText` を stdin に流して
stdout を期待値と正規化比較（全件 pass かつ exit 0 で正答）、`qa` は提出文字列を expectedOutput と比較。
出力正規化は CRLF/CR→LF 統一 + 末尾の空白/改行除去（行内空白は厳密一致 / Go 版の `TrimRight(" \t\n")` と同等）。
一覧の status（`solved` / `in_progress` / 未着手`""`）と stats（提出総数 / 正答ユーザー数）は batch クエリで
取得し N+1 を避ける。`CodeExecutor` は採点でテストケース入力を渡すため `execute(language, code, stdin)` を
追加（プレイグラウンド用に stdin 無しの 2 引数 default も残す）。

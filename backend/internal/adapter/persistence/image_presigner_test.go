...
// 表示 URL に配信ドメインを含めないことを固定する。
//
// 絶対 URL で保存していると、配信ドメインを変えるたびに過去の画像が全て参照不能になる
// （実害あり）。ここが絶対 URL に戻ると同じ障害が再発するため、
...
func Test_画像の表示URLに配信ドメインを含めないこと(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			publicURL: func(t *testing.T) string {
				t.Helper()
				out, err := NewStubRichTextImagePresigner("bucket").Generate(ctx, 7, "image/png", 1024)
				require.NoError(t, err)
...
func Test_アップロード先URLは絶対URLのままであること(t *testing.T) {
	...
	richText, err := NewStubRichTextImagePresigner("bucket").Generate(ctx, 7, "image/png", 1024)
	require.NoError(t, err)

...
type ListSpaceMemberGrantFactsParams struct {
	WorkspaceID uuid.UUID
	ID          uuid.UUID
}

type ListSpaceMemberGrantFactsRow struct {
	UserID    int64
	Name      string
	AvatarUrl string
	Role      string
...
// ここで明示的に絞る必要がある）。
func (q *Queries) ListSpaceMemberGrantFacts(ctx context.Context, arg ListSpaceMemberGrantFactsParams) ([]ListSpaceMemberGrantFactsRow, error) {
	rows, err := q.db.QueryContext(ctx, listSpaceMemberGrantFacts, arg.WorkspaceID, arg.ID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListSpaceMemberGrantFactsRow{}
	for rows.Next() {
		var i ListSpaceMemberGrantFactsRow
		if err := rows.Scan(
			&i.UserID,
			&i.Name,
			&i.AvatarUrl,

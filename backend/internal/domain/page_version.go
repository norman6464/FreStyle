...
// ErrPageVersionNotFound は対象の版が存在しない（無い seq・別ページ・別テナントのもの）
// ときに返す。
...
type PageVersion struct {
	...
	// （commentThreadResponse の CreatedBy と同じ役割分担）。
	AuthorUserID uint64 `json:"-"`
	// Note は版に添えた任意のメモ。空文字・空白のみは保存しない
	// （ValidateVersionNote が nil へ正規化する）。
...
//   - 空文字・空白のみは nil へ正規化して返す（エラーにしない — 「メモを入力しないで送る」
//     操作を利用者にエラーとして見せる理由が無いため）

...
//
// 来訪者は kind='share_link' の Principal として扱う（PrincipalID）。主体の種類を 1 本に
// 揃えておくと、権限解決の入口が主体ごとに分岐しない。「そのリンクで何ができるか」は
// Capability だけで決まる。来訪者はワークスペースに所属しないので、付与の 3 段はそもそも届かない。
//
...
type ShareLink struct {
	...
	// RevokedAt は失効日時。nil なら有効。失効は行を消さず日付で残す（誰がいつ止めたかを
	// 追えるように）。

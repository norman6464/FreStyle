...
// エンドポイント単位で確かめられる。
func docTenantRouter(db *gorm.DB, user *domain.User) *gin.Engine {
...
func TestDocumentTenantIsolation_Integration(t *testing.T) {
	...
	docRepo := persistence.NewRichDocumentRepository(db)
	...
	t.Run("取得: 別会社のユーザーは他社の公開文書を読めない(404)", func(t *testing.T) {
		w := docTenantGet(t, docTenantRouter(db, userB), "/documents/"+publicA.ID)
		require.Equal(t, http.StatusNotFound, w.Code, w.Body.String())
	})

	t.Run("取得: 同一会社の別ユーザーは公開文書を読める(200)", func(t *testing.T) {
		w := docTenantGet(t, docTenantRouter(db, peerA), "/documents/"+publicA.ID)
		require.Equal(t, http.StatusOK, w.Code, w.Body.String())
	})

	t.Run("取得: 同一会社の別ユーザーでも非公開は読めない(404)", func(t *testing.T) {
		w := docTenantGet(t, docTenantRouter(db, peerA), "/documents/"+privateA.ID)
		require.Equal(t, http.StatusNotFound, w.Code, w.Body.String())
	})

	t.Run("取得: 会社不明(NULL)の公開文書は所有者以外から読めない(404)", func(t *testing.T) {
		for _, viewer := range []*domain.User{peerA, userB} {
			w := docTenantGet(t, docTenantRouter(db, viewer), "/documents/"+publicNullCompany.ID)
			require.Equal(t, http.StatusNotFound, w.Code, w.Body.String())
		}
	})

	t.Run("取得: 所有者は会社不明(NULL)の自分の文書を読める(200)", func(t *testing.T) {
		w := docTenantGet(t, docTenantRouter(db, loneOwner), "/documents/"+publicNullCompany.ID)
		require.Equal(t, http.StatusOK, w.Code, w.Body.String())
	})

	t.Run("取得: 所有者は文書の会社が自分の会社と食い違っても読める(200)", func(t *testing.T) {
		w := docTenantGet(t, docTenantRouter(db, userB), "/documents/"+staleCompanyDoc.ID)
		require.Equal(t, http.StatusOK, w.Code, w.Body.String())
	})

	t.Run("取得: 別会社のユーザーは他社の非公開文書も読めない(404)", func(t *testing.T) {
		w := docTenantGet(t, docTenantRouter(db, userB), "/documents/"+privateA.ID)
		require.Equal(t, http.StatusNotFound, w.Code, w.Body.String())
	})

	t.Run("一覧: 他社ユーザーの一覧に他社の文書は出ない", func(t *testing.T) {
		w := docTenantGet(t, docTenantRouter(db, userB), "/documents")
		require.Equal(t, http.StatusOK, w.Code, w.Body.String())
		var rows []struct {
			ID string `json:"id"`
		}
	...
	t.Run("一覧: 同一会社の別ユーザーの公開文書も一覧には出ない(owner スコープ)", func(t *testing.T) {
		w := docTenantGet(t, docTenantRouter(db, peerA), "/documents")
		require.Equal(t, http.StatusOK, w.Code, w.Body.String())
		require.Equal(t, "[]", w.Body.String())
	})

	t.Run("一覧: 所有者は自分の文書を会社の値によらず取れる", func(t *testing.T) {
		w := docTenantGet(t, docTenantRouter(db, loneOwner), "/documents")
		require.Equal(t, http.StatusOK, w.Code, w.Body.String())
		var rows []struct {
			ID string `json:"id"`
		}

...
// Test_ラベル一式_作成一覧更新削除付け外し はラベル（段 4）の一連の流れを HTTP 経由で確かめる。
...
func Test_ラベル一式_作成一覧更新削除付け外し(t *testing.T) {
	...
	// 1) 作成。
	w := f.do(t, http.MethodPost, spaceBase+"/labels", `{"name":"緊急","color":"#FF0000"}`)
	...
	w = f.do(t, http.MethodPost, spaceBase+"/labels", `{"name":"  緊急  ","color":"#00ff00"}`)
	assert.Equal(t, http.StatusConflict, w.Code, "空白・大文字小文字違いの重複は409")

	// 3) 一覧。
	w = f.do(t, http.MethodGet, spaceBase+"/labels", "")
	require.Equal(t, http.StatusOK, w.Code)
	list := decodeJSON[map[string][]map[string]any](t, w)
	require.Len(t, list["labels"], 1)

	// 4) 更新。
	w = f.do(t, http.MethodPut, spaceBase+"/labels/"+labelID, `{"name":"重大","color":"#0000ff"}`)
	require.Equal(t, http.StatusOK, w.Code, w.Body.String())
	updated := decodeJSON[map[string]any](t, w)
	assert.Equal(t, "重大", updated["name"])

	// 5) チケットへ付与し、詳細応答に載る。
	target := f.tickets.addTicket(domain.Ticket{ID: "ticket-labels-1", WorkspaceID: kbWorkspaceID, SpaceID: kbSpaceID, Title: "対象チケット"})
	...
	w = f.do(t, http.MethodGet, ticketAPIBase+"/spaces/"+kbSpaceID+"/tickets?label="+labelID, "")
	require.Equal(t, http.StatusOK, w.Code)
	filtered := decodeJSON[map[string][]map[string]any](t, w)
	require.Len(t, filtered["tickets"], 1, "?label= で絞り込める")

	w = f.do(t, http.MethodGet, ticketAPIBase+"/spaces/"+kbSpaceID+"/tickets?label=00000000-0000-0000-0000-000000000000", "")
	require.Equal(t, http.StatusOK, w.Code)
	...
	// 8) 削除。
	w = f.do(t, http.MethodDelete, spaceBase+"/labels/"+labelID, "")
	require.Equal(t, http.StatusNoContent, w.Code)
	w = f.do(t, http.MethodGet, spaceBase+"/labels", "")
	require.Equal(t, http.StatusOK, w.Code)
...
func Test_ラベル_閲覧のみでは作成403だが一覧は見える(t *testing.T) {
	...
	w := f.do(t, http.MethodPost, spaceBase+"/labels", `{"name":"x","color":"#2f6b47"}`)
...
func Test_ラベル_閲覧のみでは更新削除付け外しも403(t *testing.T) {
	...
	spaceBase := ticketAPIBase + "/spaces/" + kbSpaceID
	w := editor.do(t, http.MethodPost, spaceBase+"/labels", `{"name":"x","color":"#2f6b47"}`)
	require.Equal(t, http.StatusCreated, w.Code)
	created := decodeJSON[map[string]any](t, w)
	labelID := created["id"].(string)
	target := editor.tickets.addTicket(domain.Ticket{ID: "ticket-labels-viewer", WorkspaceID: kbWorkspaceID, SpaceID: kbSpaceID, Title: "x"})

	viewer := ticketFixtureSameRepo(t, editor, kbUserID+1, domain.GrantRoleViewer)

	w = viewer.do(t, http.MethodPut, spaceBase+"/labels/"+labelID, `{"name":"y","color":"#2f6b47"}`)
	assert.Equal(t, http.StatusForbidden, w.Code)
	w = viewer.do(t, http.MethodDelete, spaceBase+"/labels/"+labelID, "")
	assert.Equal(t, http.StatusForbidden, w.Code)
...
func Test_ラベル_存在しないラベルの更新削除は404(t *testing.T) {
	...
	w := f.do(t, http.MethodPut, spaceBase+"/labels/no-such-label", `{"name":"y","color":"#2f6b47"}`)
	assert.Equal(t, http.StatusNotFound, w.Code)
	w = f.do(t, http.MethodDelete, spaceBase+"/labels/no-such-label", "")
	assert.Equal(t, http.StatusNotFound, w.Code)
}
...
// 違うスペースのラベルをチケットへ付けようとすると「無い」と同じ扱い（404）になる。
func Test_ラベル_違うスペースのラベルはチケットへ付けられない(t *testing.T) {
	f := newTicketFixture(kbUserID, domain.GrantRoleEditor)
	target := f.tickets.addTicket(domain.Ticket{ID: "ticket-labels-2", WorkspaceID: kbWorkspaceID, SpaceID: kbSpaceID, Title: "x"})
	f.tickets.labels["label-other"] = &domain.Label{ID: "label-other", WorkspaceID: kbWorkspaceID, SpaceID: "other-space", Name: "他スペース", Color: "#2f6b47"}

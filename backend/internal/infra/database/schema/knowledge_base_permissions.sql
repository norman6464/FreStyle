...
-- space_grants / page_restrictions / share_links）の DDL。
--
...
--   1. 対象ページ自身と祖先のうち、そのケイパビリティの制限を持つ「最も近い」ページを 1 つ選ぶ
--      （page_paths の depth が最小のもの。無ければ grants の既定に従う）
--   2. その 1 段だけを見て deny → allow の順に判定する。deny が allow に勝つ
--   3. どちらにも自分が載っていないとき、その段に allow 行が 1 つでもあれば「許可リスト」が
--      効いている状態なので不許可。deny 行しか無ければ「名指しの除外」なので既定に戻る
--
-- 3 の場が要るのは、allow と deny で意味が逆だから。allow を 1 つ足した瞬間に
-- 「載っていない人は入れない」に切り替わり（限定公開）、deny だけの段は

...
table "courses" {
  ...
  foreign_key "fk_courses_workspace" {
    columns     = [column.workspace_id]
    ref_columns = [table.workspaces.column.id]
    on_update   = NO_ACTION
    on_delete   = NO_ACTION
  }
  index "idx_courses_workspace_id" {
    columns = [column.workspace_id]
  }
  # コースの権限（course_grants）から「同じワークスペースのコース」を複合 FK で指すための
  # 足場。id は既に PK なので、この一意制約が新しく防ぐ重複は無い。
  unique "uq_courses_workspace_id" {
...
table "audit_events" {
  ...
  index "idx_audit_events_actor_id" {
...
table "user_chapter_progress" {
  ...
  index "ux_user_chapter_progress" {
    unique  = true
    columns = [column.user_id, column.chapter_id]
  }
  index "idx_user_chapter_progress_course_id" {
    columns = [column.course_id]
  }
}

# 章の閲覧記録。PK = (user_id, chapter_id)。upsert で last_viewed_at / view_count を更新する。
table "user_chapter_views" {
  schema = schema.public
  column "user_id" {
    null = false
    type = bigint
...
table "score_cards" {
  ...
  column "logical_score" {
    null = true
    type = numeric
  }
  column "consideration_score" {
    null = true
    type = numeric
  }
...
table "course_chapters" {
  ...
  # 所属の正本（company_id は撤去済み）。workspace_id を NOT NULL にしていないのは、教材の投入 SQL
  # （別リポで生成する）がこの列を書いているかをここから確かめられないため。
  # 確かめてから別途締める。
  column "workspace_id" {
    null = true
    type = uuid
  ...
  foreign_key "fk_course_chapters_workspace" {
    columns     = [column.workspace_id]
    ref_columns = [table.workspaces.column.id]
    on_update   = NO_ACTION
    on_delete   = NO_ACTION
  ...
  # テナントの一致は下の uq_course_chapters_workspace_id と、権限側から張る複合 FK が受け持つ。
  foreign_key "fk_course_chapters_course" {
    columns     = [column.course_id]
    ref_columns = [table.courses.column.id]
    on_update   = NO_ACTION
    on_delete   = CASCADE
  }
  index "idx_course_chapters_course_id" {
    columns = [column.course_id]
  }
  index "idx_course_chapters_workspace_id" {
    columns = [column.workspace_id]
  }
  # 章の権限（chapter_grants）から「同じワークスペースの章」を複合 FK で指すための足場。
  unique "uq_course_chapters_workspace_id" {
    columns = [column.workspace_id, column.id]
  }
}
...
# =====================================================================
# コース / 教材の権限（course_grants / chapter_grants）
# =====================================================================
#
...
table "course_grants" {
  ...
  primary_key {
    columns = [column.workspace_id, column.course_id, column.principal_id]
  }
  # 複合にすることで「同じワークスペースのコース」しか指せない（テナント跨ぎの付与を塞ぐ）。
  foreign_key "fk_course_grants_course" {
    columns     = [column.workspace_id, column.course_id]
    ref_columns = [table.courses.column.workspace_id, table.courses.column.id]
    on_update   = NO_ACTION
    on_delete   = CASCADE
  }
  foreign_key "fk_course_grants_principal" {
    columns     = [column.workspace_id, column.principal_id]
    ref_columns = [table.principals.column.workspace_id, table.principals.column.id]
    on_update   = NO_ACTION
    on_delete   = CASCADE
  }
  index "idx_course_grants_principal" {
    columns = [column.workspace_id, column.principal_id]
  }
  check "ck_course_grants_role" {
    expr = "(role)::text = ANY (ARRAY[('admin'::character varying)::text, ('editor'::character varying)::text, ('commenter'::character varying)::text, ('viewer'::character varying)::text])"
  }
}
...
table "chapter_grants" {
  ...
  primary_key {
    columns = [column.workspace_id, column.chapter_id, column.principal_id]
  }
  foreign_key "fk_chapter_grants_chapter" {
    columns     = [column.workspace_id, column.chapter_id]
    ref_columns = [table.course_chapters.column.workspace_id, table.course_chapters.column.id]
    on_update   = NO_ACTION
    on_delete   = CASCADE
  }
  foreign_key "fk_chapter_grants_principal" {
    columns     = [column.workspace_id, column.principal_id]
    ref_columns = [table.principals.column.workspace_id, table.principals.column.id]
    on_update   = NO_ACTION
    on_delete   = CASCADE
  }
  index "idx_chapter_grants_principal" {
    columns = [column.workspace_id, column.principal_id]
  }
  check "ck_chapter_grants_role" {
    expr = "(role)::text = ANY (ARRAY[('admin'::character varying)::text, ('editor'::character varying)::text, ('commenter'::character varying)::text, ('viewer'::character varying)::text])"
  }
}

...
func Test_監査記録_入力をイベントに詰めて保存する(t *testing.T) {
	// 保存された内容を検証したいので、Run でクロージャに退避する。
	var recorded []domain.AuditEvent
	repo := &mockAuditRepo{}
	repo.On("Record", mock.Anything, mock.Anything).
		Run(func(args mock.Arguments) {
			recorded = append(recorded, *args.Get(1).(*domain.AuditEvent))
		}).Return(nil)
...
func Test_監査記録_保存失敗を伝播(t *testing.T) {
	...
	repo := &mockAuditRepo{}
	repo.On("Record", mock.Anything, mock.Anything).Return(wantErr)
	uc := usecase.NewRecordAuditEventUseCase(repo)
	err := uc.Execute(context.Background(), usecase.RecordAuditEventInput{})
...
func Test_監査ログ一覧_新しい順で返す(t *testing.T) {
	repo := &mockAuditRepo{}
	repo.On("ListRecent", mock.Anything, mock.Anything).
	...
	repo.AssertNumberOfCalls(t, "ListRecent", 1)
}

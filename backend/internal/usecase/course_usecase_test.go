...
func Test_コース_更新_別会社は禁止(t *testing.T) {
	crepo, cstore := courseRepo(courseFakeConfig{get: &domain.Course{ID: 1, CompanyID: 10, WorkspaceID: strPtr(wsA), Title: "old"}})
	mrepo, _ := materialRepo(materialFakeConfig{})
	uc := usecase.NewCourseUseCase(crepo, mrepo)
...
func Test_コース_削除_traineeは禁止(t *testing.T) {
	crepo, _ := courseRepo(courseFakeConfig{get: &domain.Course{ID: 1, CompanyID: 10, WorkspaceID: strPtr(wsA)}})
	mrepo, _ := materialRepo(materialFakeConfig{})
	uc := usecase.NewCourseUseCase(crepo, mrepo)
...
func Test_コース_削除_自社管理者は教材も連鎖削除(t *testing.T) {
	crepo, cstore := courseRepo(courseFakeConfig{get: &domain.Course{ID: 1, CompanyID: 10, WorkspaceID: strPtr(wsA)}})
...
func Test_コース_更新_カテゴリを変更できる(t *testing.T) {
	crepo, _ := courseRepo(courseFakeConfig{get: &domain.Course{ID: 1, CompanyID: 10, WorkspaceID: strPtr(wsA), Category: domain.CourseCategoryDevBasics}})
	mrepo, _ := materialRepo(materialFakeConfig{})
	uc := usecase.NewCourseUseCase(crepo, mrepo)
	got, err := uc.Update(context.Background(), usecase.UpdateCourseInput{
		ID: 1, ActorWorkspace: domain.WorkspaceRefOf(wsA), ActorRole: domain.RoleCompanyAdmin,
...
func Test_コース_更新_言語を変更できる(t *testing.T) {
	crepo, _ := courseRepo(courseFakeConfig{get: &domain.Course{ID: 1, CompanyID: 10, WorkspaceID: strPtr(wsA), Language: "go"}})
	mrepo, _ := materialRepo(materialFakeConfig{})
	uc := usecase.NewCourseUseCase(crepo, mrepo)
	got, err := uc.Update(context.Background(), usecase.UpdateCourseInput{
		ID: 1, ActorWorkspace: domain.WorkspaceRefOf(wsA), ActorRole: domain.RoleCompanyAdmin,

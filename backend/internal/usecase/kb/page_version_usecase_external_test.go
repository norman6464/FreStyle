...
func Test_版を残す_snapshotがあればそれをdocとして使う(t *testing.T) {
	...
	versionRepo.On("CreateVersionIfDue", mock.Anything, kbWS, kbPage, snapshotDoc, kbEditorUserID, (*string)(nil), true).
		Return(true, created, nil)
	uc := kb.NewCreateExplicitPageVersionUseCase(versionRepo, kbRepo)
...
func Test_版を残す_snapshotが無ければブロックから組み立てる(t *testing.T) {
	...
	versionRepo := &mockPageVersionRepo{}
	var gotDoc string
	versionRepo.On("CreateVersionIfDue", mock.Anything, kbWS, kbPage, mock.Anything, kbEditorUserID, (*string)(nil), true).
...
func Test_復元_ForceVersionは必ずtrue(t *testing.T) {
	...
	kbRepo.On("ReplacePageBlocks", mock.Anything, kbWS, kbPage, mock.Anything, mock.Anything).Return(nil)
	kbRepo.On("GetPageSnapshot", mock.Anything, kbWS, kbPage).

...
// KbImagePresigner はナレッジのページ画像用 S3 presigned URL を発行する。
//
// 認可（閲覧・編集権限、key とページの結びつき）は呼び出し側（usecase）が済ませている前提で、
// ここは署名の発行だけを担う。これは新しい「表」ではなく S3 という外部サービスへの port なので、
// KnowledgeBaseRepository（pages / blocks への port）には含めない。

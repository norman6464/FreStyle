...
//
// 権限判定はここに含めない。チケットの実効権限はページを介さない「スペース単位」の判定
// （設計 Ⅳ-H）で、既存の KnowledgeBasePermissionRepository.SpacePermissionFactsForUser が
// そのまま使える。usecase/ticket が両方の repository に依存し、チケット→スペースの解決だけを
...
type TicketRepository interface {
	...
	// 「その行が在るか・どのスペースか」だけが要る内部用途に使う。画面へ返す取得は
	...
	// FindActiveTicketPosition は move の before/after 指定チケットが、指定スペースの
	// 現役の兄弟であることを検証する。

...
func Test_AIメッセージ送信ストリーム_既存セッション_トークン配信と最終保存(t *testing.T) {
	...
	uc := usecase.NewSendAiMessageStreamUseCase(sessionRepo, msgRepo, bc, nil, &nopActivityRepo{})
...
func Test_AIメッセージ送信ストリーム_添付あり_blob取得とメタ保存(t *testing.T) {
	...
	uc := usecase.NewSendAiMessageStreamUseCase(sessionRepo, msgRepo, bc, dl, &nopActivityRepo{})
	ch, err := uc.Execute(context.Background(), usecase.SendAiMessageInput{
		UserID:    7,
		SessionID: 5,
		Content:   "この画像なに？",

...
func Test_状態作成_不正な値は拒否(t *testing.T) {
	...
	_, err := uc.Execute(ctx, ticket.CreateTicketStatusInput{WorkspaceID: tkWS, SpaceID: tkSpace, Name: "", Category: domain.TicketStatusCategoryTodo, Color: "#2f6b47"})
	require.ErrorIs(t, err, domain.ErrInvalidTicketName, "空の名前は拒否")
	_, err = uc.Execute(ctx, ticket.CreateTicketStatusInput{WorkspaceID: tkWS, SpaceID: tkSpace, Name: "  ", Category: domain.TicketStatusCategoryTodo, Color: "#2f6b47"})
	require.ErrorIs(t, err, domain.ErrInvalidTicketName, "空白だけの名前も拒否")
	_, err = uc.Execute(ctx, ticket.CreateTicketStatusInput{WorkspaceID: tkWS, SpaceID: tkSpace, Name: "OK", Category: "unknown", Color: "#2f6b47"})
	require.ErrorIs(t, err, domain.ErrInvalidTicketStatusCategory, "未知のcategoryは拒否")
	_, err = uc.Execute(ctx, ticket.CreateTicketStatusInput{WorkspaceID: tkWS, SpaceID: tkSpace, Name: "OK", Category: domain.TicketStatusCategoryTodo, Color: "url(evil)"})
	require.ErrorIs(t, err, domain.ErrInvalidTicketColor, "不正な色は拒否")
}

func Test_状態作成_色は正規化してから保存する(t *testing.T) {
	repo := &mockTicketRepo{}
	repo.On("LastActiveTicketStatusPosition", mock.Anything, tkWS, tkSpace).Return("a0", nil)
	...
	_, err := ticket.NewCreateTicketStatusUseCase(repo).Execute(context.Background(), ticket.CreateTicketStatusInput{
		WorkspaceID: tkWS, SpaceID: tkSpace, Name: "レビュー中", Category: domain.TicketStatusCategoryInProgress, Color: "#2F6B47",
	})
...
func Test_状態アーカイブ_使用中なら拒否(t *testing.T) {
	repo := &mockTicketRepo{}
	repo.On("CountActiveTicketsByStatus", mock.Anything, tkWS, tkSpace, "status-1").Return(int64(3), nil)

	err := ticket.NewArchiveTicketStatusUseCase(repo).Execute(context.Background(), ticket.ArchiveTicketStatusInput{
		WorkspaceID: tkWS, SpaceID: tkSpace, StatusID: "status-1",
	})
...
func Test_状態アーカイブ_未使用なら実行する(t *testing.T) {
	repo := &mockTicketRepo{}
	repo.On("CountActiveTicketsByStatus", mock.Anything, tkWS, tkSpace, "status-1").Return(int64(0), nil)
	repo.On("ArchiveTicketStatus", mock.Anything, tkWS, tkSpace, "status-1").Return(nil)
...
func Test_状態復元_positionを末尾へ(t *testing.T) {
	repo := &mockTicketRepo{}
	repo.On("LastActiveTicketStatusPosition", mock.Anything, tkWS, tkSpace).Return("a0", nil)
	repo.On("RestoreTicketStatus", mock.Anything, tkWS, tkSpace, "status-1", mock.MatchedBy(func(pos string) bool {
		return pos > "a0"
	})).Return(nil)
...
func Test_状態復元_名前衝突は409相当(t *testing.T) {
	repo := &mockTicketRepo{}
	repo.On("LastActiveTicketStatusPosition", mock.Anything, tkWS, tkSpace).Return("a0", nil)
	repo.On("RestoreTicketStatus", mock.Anything, tkWS, tkSpace, "status-1", mock.Anything).

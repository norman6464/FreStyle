...
func Test_種別作成_不正な値は拒否(t *testing.T) {
	...
	_, err := uc.Execute(ctx, ticket.CreateTicketTypeInput{WorkspaceID: tkWS, SpaceID: tkSpace, Name: "", HierarchyLevel: 0, Color: "#2f6b47"})
	require.ErrorIs(t, err, domain.ErrInvalidTicketName, "空の名前は拒否")
	_, err = uc.Execute(ctx, ticket.CreateTicketTypeInput{WorkspaceID: tkWS, SpaceID: tkSpace, Name: "OK", HierarchyLevel: 2, Color: "#2f6b47"})
	require.ErrorIs(t, err, domain.ErrInvalidTicketHierarchyLevel, "範囲外のhierarchy_levelは拒否")
	_, err = uc.Execute(ctx, ticket.CreateTicketTypeInput{WorkspaceID: tkWS, SpaceID: tkSpace, Name: "OK", HierarchyLevel: 0, Color: "not-a-color"})
	require.ErrorIs(t, err, domain.ErrInvalidTicketColor, "不正な色は拒否")
}

func Test_種別作成_正常に作る(t *testing.T) {
	repo := &mockTicketRepo{}
	repo.On("LastActiveTicketTypePosition", mock.Anything, tkWS, tkSpace).Return("", nil)
	...
	_, err := ticket.NewCreateTicketTypeUseCase(repo).Execute(context.Background(), ticket.CreateTicketTypeInput{
		WorkspaceID: tkWS, SpaceID: tkSpace, Name: "バグ", HierarchyLevel: 0, Color: "#9A3B2E",
	})
...
func Test_種別アーカイブ_使用中なら拒否(t *testing.T) {
	repo := &mockTicketRepo{}
	repo.On("CountActiveTicketsByType", mock.Anything, tkWS, tkSpace, "type-1").Return(int64(1), nil)

	err := ticket.NewArchiveTicketTypeUseCase(repo).Execute(context.Background(), ticket.ArchiveTicketTypeInput{
		WorkspaceID: tkWS, SpaceID: tkSpace, TypeID: "type-1",
	})

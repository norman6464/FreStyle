...
- 既表示チャンクは React が DOM ノードを再利用するためアニメが再生し直されない。新しく mount した span だけが
  ...
  DOM ノード同一性でガード）。
- **画面外で mount したチャンクはフェードを保留し、スクロールで見えた瞬間に再生する（FRESTYLE-153）**。
  CSS アニメーションは mount 時に走るため、自動スクロール追従をやめた（FRESTYLE-149）後は本文が画面外で
  フェードを再生し終えてしまい、自分でスクロールして読むとアニメーションが一切見えなかった。
  [useFadeOnVisible](../frontend/src/hooks/useFadeOnVisible.ts) が `useLayoutEffect` で新規 span を走査し、
  画面外のものに `data-fade="defer"`（`animation: none; opacity: 0`）を付けて IntersectionObserver で監視、
  画面に入った時点で `data-fade="now"` に変えてフェードを走らせる。
  安全側の設計: `IntersectionObserver` が無い環境では何も保留しない（従来どおり mount 時フェード）／
  保留するのは画面外の要素だけ／応答完了後はプラグインごと外れて `.fade-seg` が消えるため、
  保留されたまま不可視で固定される要素は残らない。`prefers-reduced-motion: reduce` では保留自体を無効化する。
...
### スクロール（FRESTYLE-149 でストリーミング中の追従を撤去）

「本文が下端いっぱいになると勝手にスクロールする」というユーザー要望で、**ストリーミング中の自動追従をやめた**。

- 最下部へ寄せるのは **メッセージ件数が変わったとき**（送信・セッション読込）だけ（`useEffect(..., [messages.length])`）。
  トークン追記は件数不変で本文だけ伸びるため発火せず、本文が下端を越えても追従しない。
- FRESTYLE-146 で入れていた ResizeObserver（放出中の高さ成長を追う下端追従）は撤去。
- 「最下部へ戻る」ボタン（上へスクロールしたときのみ表示）と wheel/touchmove による追従停止判定は維持。

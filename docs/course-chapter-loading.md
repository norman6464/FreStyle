...
## 完了トグルの配置（FRESTYLE-100 → 119 で固定を解除）

受講者向け本文（ReadOnlyDetail）のメタ行（最終更新日・目次トグル・完了トグル）は
**通常の行**として本文と一緒にスクロールする。FRESTYLE-100 で `sticky top-0` により
固定表示していたが、「スクロールしたらヘッダーが固定されるのは不要」というユーザー要望で
FRESTYLE-119 で解除した。

- スクロール途中の完了操作は、本文末尾の大きい完了ボタン（FRESTYLE-100 で追加・維持）で行える
- 「次の章へ」の読了導線もそのまま
...
## 閲覧ビューのレイアウト（FRESTYLE-118）

受講者の閲覧ビューは「灰青背景 + 白カード」の読み物レイアウト（ユーザー指定の配色。現値は design-tokens.md 参照）:

- スクロール領域全体が灰青、タイトル + メタ + 本文 + 末尾ナビは白カード（角丸・枠・薄い影）。
  中央寄せ max-width なので左右は自然に余白になる
- **章一覧は左パネルから右サイドバー（目次の下）へ移動**。右サイドバーは lg 以上で sticky 表示され、
  「目次カード」と「章カード（コース一覧リンク / コース名 / 進捗バー / 章リスト = 完了はチェック・
  現在章はハイライト + aria-current）」の 2 枚の白カード。`ChapterNav` コンポーネント
- 受講者ではデスクトップの左 SecondaryPanel を出さない（`md:hidden` ラッパ）。モバイル（md 未満）は
  右サイドバーが無いため、従来のドロワーを章切替の導線として残す
- 管理者（編集）ビューは従来どおり左パネル + エディタ
- 章本文取得中のスケルトンも同じ配色（切り替わり時に背景が変わらないように）
- 本文内の画像（図）は figure + img + figcaption で表示し、**リンクにはしない**
  （かつては「クリックで原寸を別タブ表示」だったが、画像 URL が別タブで開いて学習が中断される
  というユーザー要望で FRESTYLE-125 にて除去）
- **body の背景も同色に統一済み**（FRESTYLE-119 で `--color-surface` を変更。
  教材ペインだけ色が違う食い違いを解消し、全画面が「灰青ボディ + 白カード」の構成）

## タイトルを白カードの外へ（FRESTYLE-131）

記事投稿サイトのように、章タイトル（と最終更新日・目次トグル・完了トグルのメタ）を
**白カードの外・上**（灰青背景の上）に中央寄せで置く。白カードは本文 + 末尾ナビだけを含む。

- 教材本文は規約で先頭が `# タイトル`（= `material.title` と同じ）。ヘッダーで `material.title` を
  大きく表示するため、本文側の**先頭 h1 は `stripLeadingTitle` で除去**してカード内の二重表示を防ぐ
- `stripLeadingTitle` は「先頭の空行を飛ばした最初の非空行が h1 のときだけ」1 個を除去する。
  コードブロック内の `# コメント` や 2 個目以降の見出しには触れない
- 目次（`MarkdownTableOfContents`）にも同じ除去後の本文を渡し、目次先頭からもタイトルを外す
  （タイトルはヘッダーで大きく出るため）
- `MaterialSkeleton` も同じ配置（カード上にタイトル + メタのプレースホルダ）に合わせて跳ねを防ぐ

## グローバルヘッダーもスクロールで流す（FRESTYLE-122）

「スクロールしてもアプリのヘッダーが固定されたまま」というユーザー要望への対応。
[AppShell](../frontend/src/components/layout/AppShell.tsx) に**ドキュメントスクロール**モードを追加した:

- **受講者（管理ロール以外）× `/courses/:id`** のときだけ、ヘッダーと main を 1 つの
  スクロールコンテナ（`#app-scroll` / `data-app-scroll`）に入れる。スクロールするとヘッダーが
  本文と一緒に画面外へ流れる
- チャット / ノート / コース編集などのパネル型ページは main の固定高さ（`flex-1 min-h-0 overflow-auto`）に
  依存しているため、**従来レイアウトのまま**。管理ロールの `/courses/:id`（編集ビュー）も従来どおり
- これに伴い ReadOnlyDetail / MaterialSkeleton の内部スクロール（`overflow-y-auto`）を撤去し、
  章切替時の先頭スクロールは `closest('[data-app-scroll]')` で外側のコンテナに対して行う
  （見つからない場合は自要素にフォールバック = 単体テストでも安全）
- 「トップへ戻る」ボタン（ScrollToTop）の `targetId` もモードに応じて `app-scroll` / `main-content` を切り替える
- 右サイドバー（目次 + 章ナビ）の `sticky top-6` は最寄りのスクロール祖先が外側コンテナに変わるだけで
  そのまま機能する（ヘッダーが流れた後は画面上部に張り付く）

## サイドバーのカード内スクロール（FRESTYLE-144）

章が長いと目次・章一覧が縦に伸び続けてサイドバーが間延びするため、**各カードを一定の高さに収めて
溢れたぶんはカード内でスクロール**させる（記事投稿サイトの目次のように、カードごとに独立した
スクロールバーが出る）。

- サイドバー（`aside` 内の sticky コンテナ）を `max-h-[calc(100vh-3rem)]`（`top-6` = 1.5rem 分を上下に確保）で
  ビューポート高さに収め、`flex flex-col gap-4` の縦積みにする。両カードは `flex min-h-0 flex-col` で、
  内容ぶんの高さを取りつつ、合計が収まらないときは flex-shrink で縮む（項目が多いカードほど多く縮む）
- カード内で **固定する部分とスクロールする部分を分ける**:
  - 目次（`MarkdownTableOfContents`）: 見出し「目次」は `flex-shrink-0` で固定、`<ul>` を
    `flex-1 min-h-0 overflow-y-auto` にしてリストだけスクロール
  - 章一覧（`ChapterNav`）: コース名・進捗バーのヘッダーは `flex-shrink-0`、章リスト（`<nav>`）を
    `flex-1 min-h-0 overflow-y-auto` にしてリストだけスクロール
- 短い章では溢れないのでスクロールバーは出ない（`overflow-y-auto`）。スクロールバーの見た目は
  `App.css` のグローバル定義（幅 8px・角丸サム `--scrollbar-thumb`）をそのまま使う
- `pr-1` でスクロールバーとテキストの重なりを避ける。モバイル（lg 未満）は右サイドバー自体が
  非表示なので影響なし

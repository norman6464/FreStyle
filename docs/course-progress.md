...
## 一覧の視認性改善（FRESTYLE-114）

ユーザー要望 3 点への対応:

- **完了コースの見た目**: 受講者視点で `completedCount >= materialCount`（かつ章 > 0）のカードは、
  濃緑の「完了」バッジ + 薄緑の背景/枠のアクセントで一目で分かる。管理ロールには出さない
  （進捗は受講者個人のもので、管理者ビューの一覧に個人進捗は表示しない従来方針のまま）
- **残り章数の明示**: `CourseProgressBar` のラベルを進行中は `{completed}/{total}（{pct}%・残り {R} 章）`、
  全章完了は `すべて完了（{total} 章）`（濃緑バー）にした。コース詳細の左パネルにも同じ表示が出る
- **コースの言語バッジ**: `courses.language` 列を追加（varchar(50)・既定 ''。AutoMigrate が列追加）。
  カードのメタ行に演習と同じ `LanguageBadge` で表示し、空なら非表示（言語が主題でないコース）。
  色の正本は `src/constants/languageBadgeClasses.ts` に一本化し、演習の言語バッジと同じ技術は
  同じ色に見える（go=シアン / docker=空色 / postgresql=teal / terraform=violet / aws=amber /
  openapi=emerald / web=rose / linux=slate 等）。フォームの選択肢は `src/constants/courseLanguages.ts`
- 既存 22 コースの言語は `backend/migrations/0008_course_languages_backfill.sql`（冪等・列存在チェック付き）
  で backfill する。デプロイ（列作成）→ backfill の順で適用する

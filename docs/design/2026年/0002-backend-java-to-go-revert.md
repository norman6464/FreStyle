...
| PR | 内容 | フェーズ | 破壊性 |
|---|---|---|---|
| 1 | G1: code/execute に java 追加 + Dockerfile に JDK | A | 無（Go は未デプロイ） |
| 2 | G2: auth ルートを REST 化（frontend 整合） | A | 無 |
| 3 | G3: courses/teaching-materials 復元 | A | 無 |
| 4 | G4: learning-reports 復元 | A | 無 |
| 5 | G5: company settings(AI トグル) 移植 + companies(V13 相当 AutoMigrate) | A | 無 |
| 6 | G6: 採点パリティ検証・補完 + ARCHITECTURE/README 更新 | A | 無 |
| 7 | ECS image を Go へ切替（infra） | B | **要 GO** |
| 8 | backend-java/ 削除 + ECR Java イメージ削除 + Java CI 削除 | C | **要 GO** |

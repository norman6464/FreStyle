...
### CF_API_TOKEN の作成手順（Cloudflare ダッシュボード）

1. https://dash.cloudflare.com/profile/api-tokens を開く
2. `Create Token` をクリック
3. テンプレ `Edit zone DNS` を選択
4. Permissions:
   - `Zone | DNS | Edit`
5. Zone Resources:
   - `Include | Specific zone | normanblog.com`
...
2. `normanblog.com` ドメインをクリック
3. 右側の `Overview` ページに `Zone ID` が表示される

### GitHub Secrets に登録

```bash
read -s -p "CF_API_TOKEN: " CF_API_TOKEN; echo
echo "$CF_API_TOKEN" | gh secret set CF_API_TOKEN -R norman6464/FreStyle

read -p "CF_ZONE_ID: " CF_ZONE_ID
echo "$CF_ZONE_ID" | gh secret set CF_ZONE_ID -R norman6464/FreStyle
```

> 注意: 未登録のまま scheduled-start.yml を実行すると **fail-fast で即座に exit 1** する（旧バージョンは silent fail だったが本リポは検出済み）。

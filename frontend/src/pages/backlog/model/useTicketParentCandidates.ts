...
/**
 * useTicketParentCandidates は「親を選ぶ」ピッカーの候補として、スペース内の現役チケットを
 ...
 */
export function useTicketParentCandidates(workspaceSlug: string | undefined, spaceId: string | undefined) {
  ...
  const load = useCallback(async (slug: string, space: string) => {
    ...
    setError(null);
    try {
      const list = await TicketRepository.fetchTickets(slug, space, {});

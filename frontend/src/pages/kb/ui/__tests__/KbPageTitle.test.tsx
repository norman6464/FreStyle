...
describe('KbPageTitle', () => {
  it('編集できない人には見出しとして出す（入力欄を出さない）', () => {
    render(<KbPageTitle title="設計メモ" canEdit={false} onRename={vi.fn()} />);
  ...
  it('書き換えて Enter で確定すると onRename が呼ばれる', async () => {
    ...
    render(<KbPageTitle title="無題" canEdit onRename={onRename} />);
  ...
  it('空にして確定しても改名しない（元の題名に戻す）', async () => {
    ...
    render(<KbPageTitle title="設計メモ" canEdit onRename={onRename} />);
...
describe('Enter で本文へ移る合図（onEnter）', () => {
  it('Enter で確定したとき onEnter が呼ばれる（題名が変わっていなくても）', () => {
    const onEnter = vi.fn();
    render(
      <KbPageTitle title="設計メモ" canEdit onRename={vi.fn()} onEnter={onEnter} />,
    );
  ...
  it('改名が走る場合も、失敗した場合も onEnter は 1 回だけ呼ばれる', async () => {
    ...
    const onEnter = vi.fn();
    render(
      <KbPageTitle title="設計メモ" canEdit onRename={onRename} onEnter={onEnter} />,
    );
    const input = screen.getByRole('textbox', { name: 'ページの題名' });

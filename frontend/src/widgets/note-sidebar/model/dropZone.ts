 ...
 */
export function dropZoneAt(ratio: number): NoteDropZone {
...
export function dropZoneFromEvent(rect: { top: number; height: number }, clientY: number): NoteDropZone {
  if (rect.height <= 0) return 'into';
  return dropZoneAt((clientY - rect.top) / rect.height);
}

/** toDropTarget は落下先の種類と行のページ ID を、木を動かす指定に変える。 */
export function toDropTarget(zone: NoteDropZone, pageId: string): NoteDropTarget {

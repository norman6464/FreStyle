...
import type { NoteDropTarget, NotePage, NoteSpace } from '@/entities/note';
import { toDropTarget, type NoteDropZone } from '../model/dropZone';
...
export interface NoteSpaceSectionProps {
  space: NoteSpace;
  state: NoteSpaceState | undefined;
  ...
  /** ページを作る。**失敗は投げてくる。** */
  onCreatePage: (spaceId: string, parentId?: string) => Promise<NotePage>;
  /** 題名を変える。**失敗は投げてくる。** */
  ...
  onRenameSpace: (spaceId: string, name: string) => Promise<NoteSpace>;
  /** ドラッグで動かす。**先に画面が動き、断られたら元へ戻る。失敗は投げてくる。** */
  onMovePage: (spaceId: string, pageId: string, target: NoteDropTarget) => Promise<void>;
}

/**
 * NoteSpaceSection はスペース 1 つ分の見出しと、その配下のページの木。
 *
 ...
 */
export default function NoteSpaceSection({
  space,
  state,
  workspaceSlug,
  activePageId,
  ...
  const [dropAt, setDropAt] = useState<{ pageId: string; zone: NoteDropZone } | null>(null);
  ...
  const commitRename = async (pageId: string, title: string) => {
    try {
      ...
      // 入力欄は開いたままにする（投げると NoteInlineRename がフォーカスを戻す）。
  ...
  const movePage = async (pageId: string, target: NoteDropTarget) => {
    try {
  ...
  const dropOnRow = async (pageId: string, zone: NoteDropZone) => {

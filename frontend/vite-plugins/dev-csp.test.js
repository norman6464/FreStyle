...
const HTML = [
  '<!--',
  "  - connect-src 'self' https://api.frestyle.dev : 解説コメント（書き換えない）",
  '-->',
  '<meta http-equiv="Content-Security-Policy" content="default-src \'self\';',
  " connect-src 'self' https://api.frestyle.dev; object-src 'none'\" />",
].join('\n');
...
describe('addConnectSrc', () => {
  it('meta タグの connect-src に開発用オリジンを足す', () => {
    ...
    expect(result).toContain("connect-src 'self' https://api.frestyle.dev http://localhost:8080;");
  });
